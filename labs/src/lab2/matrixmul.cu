...
{
if (fabs(P.elements[i]-reference.elements[i])>0.0001)
//printf("%i %f\n",i,P.elements[i]/reference.elements[i]);
//printf("%i %i\n",i/P.width,i % P.width);
printf("%i %f %f\n",i,P.elements[i],reference.elements[i]);
...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	dim3 gridDim(1+(P.width-1)/TILE_WIDTH,1+(P.height-1)/TILE_WIDTH);
//	dim3 gridDim(P.width/TILE_WIDTH,P.height/TILE_WIDTH);	
	dim3 blockDim(TILE_WIDTH, TILE_WIDTH);

	// Launch the device computation threads!
	MatrixMulKernel <<<gridDim, blockDim>>> (Md, Nd, Pd);
