...
{
	//allocate extra column to avoid bank conflict	
	...
	float Pvalue = 0;        
	for (int m = 0; m < 1+(M.width-1)/TILE_WIDTH; ++m)
	{
		// Read global to shared
		// Set elements outside of array to 0
	...
	// Only write elements with output array
