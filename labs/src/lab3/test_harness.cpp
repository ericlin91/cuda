...
int main(int argc, char* argv[])
{
   ...
   //store input as "one d" array - easier to think about for CopyToDeviceArray later...
   //INPUT_WIDTH is padded a la util.cpp...
   uint32_t* d_input = (uint32_t*) AllocateDeviceArray(INPUT_HEIGHT * ((INPUT_WIDTH + 128) & 0xFFFFFF80)  * sizeof(uint32_t));
   uint32_t* d_kernel_bins = (uint32_t*) AllocateDeviceArray(HISTO_HEIGHT * HISTO_WIDTH * sizeof(uint32_t));

   //move input to device
   CopyToDeviceArray(d_input, &(input[0][0]), INPUT_HEIGHT * ((INPUT_WIDTH + 128) & 0xFFFFFF80) * sizeof(uint32_t));

   //MemsetHandler(d_kernel_bins);
    /* End of setup code */

    /* This is the call you will use to time your parallel implementation */
    TIME_IT("opt_2dhisto",
            1000,
            opt_2dhisto(d_input,INPUT_HEIGHT, INPUT_WIDTH, d_kernel_bins);)

    /* Include your teardown code below (temporary variables, function calls, etc.) */

   //get solution from device
   CopyFromDeviceArray(kernel_bins, d_kernel_bins, HISTO_HEIGHT * HISTO_WIDTH * sizeof(uint32_t));

   //free device arrays
   FreeDeviceArray(d_input);
   FreeDeviceArray(d_kernel_bins);
...
/*
    ...
    for (int i=0; i<400;i +=1){
		...
		count += gold_bins[i];
		count_cuda += kernel_bins[i];

		if (gold_bins[i]!=kernel_bins[i])
		 printf("%i  gold %i    cuda %i \n",i,gold_bins[i],kernel_bins[i]);
		else
	 	 printf("%i  gold %i    cuda %i \n",i,gold_bins[i],kernel_bins[i]);
	}
printf("%i gold %i cuda total",count,count_cuda);
*/
